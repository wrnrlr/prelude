...
export function createHyperScript(r, patch) {
  function item(l,e,multiExpression) {
    if (l===null) return
    const t = r.archetype(l)
    if (t==='string') e.appendChild(r.createTextNode(l))
    else if (t==='number'||t==='bigint'||t==='boolean'||t==='date'||t==='regexp') e.appendChild(r.createTextNode(l.toString()))
  ...
  return (...args) => {
    ...
    const ret = () => {
      let tag, element, e
      const t1 = r.archetype(args[0])
      if (t1==='function') element = args.shift()
      else if (t1==='string') tag = parseTag(args.shift())
      const idx = args.findIndex(a=>r.archetype(a)==='object'), props = idx===-1 ? {} : args.splice(idx,1)[0]
      if (tag) {
        ...
        dynamic ? r.spread(e, props, r.isSVG(e), !!args.length) : r.assign(e, props, r.isSVG(e), !!args.length)
        item(args,e,multiExpression)
      } else if (element) {
        const d = Object.getOwnPropertyDescriptors(props)
        if (args) props.children = args
        for (const k in d) {
